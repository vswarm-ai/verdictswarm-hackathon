...
export default function Top200Table() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/top200");
        if (!res.ok) throw new Error("Failed to load top 200");
  ...
  const body = useMemo(() => {
    ...
    if (!rows) return <p className="text-sm text-white/60">Loading market data…</p>;
  ...
  return (
    <section className="mt-0">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold">📊 Top 200 by Market Cap</h2>
